...
41. **Build scripts**
    ...
    - Modify `Makefile` (root):
      - Ensure `.PHONY` includes `server web build clean run`.
      - Keep existing `server` and `web` targets as-is (change only if necessary to align paths).
      - Add `run` target that:
    ...
    - Implement `run` with portable shell primitives:
      - Start server in background: `(cd server && RP_ID?=${RP_ID} ORIGIN?=${ORIGIN} PORT?=${PORT} DB_PATH?=${DB_PATH} go run ./cmd/api) & SERVER_PID=$$!`
    ...
    - Test targets:
      - `make test` runs short Go tests from repo root; expect the same pass/fail as `cd server && go test -short ./...`.
      - `make test-all` runs full Go tests; allow `PKG` and `RUN` to narrow scope.
