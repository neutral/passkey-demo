...
- `RegistrationOptionsHandler(cfg, store)` — `POST /authn/passkey/registration/options` handler.
 - `RegistrationOptionsHandler(cfg, store)` — `POST /authn/passkey/registration/options` handler; emits a structured success log `reg_options` via `InfoContext` (context handler injects `correlation_id`) with `rp_id`, `origin`, `expires_at`, and `session_id_len`.
