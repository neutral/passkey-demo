...
# Key Logic
- `LoginSessionStore(capacity)`: in‑memory, concurrency‑safe store of `{challenge, rp_id, origin, expires_at}` keyed by `login_session_id`.
- `BuildLoginOptions`: 24‑byte session id, 32‑byte challenge, TTL 5m; persists session and returns JSON with `options`.
