...
- `LoginOptionsHandler`: POST endpoint that returns `rp_id`, `origin`, `uv_required=true`, `allow_credentials=[]`; emits `login_options` structured log via `InfoContext` (context handler injects `correlation_id`) with `rp_id`, `origin`, and `expires_at`.
