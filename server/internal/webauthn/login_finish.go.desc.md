...
- Logging: on verify failure, emits a single `webauthn_assert_verify` log via `LogAssertion` with `error_kind` and safe attributes (no raw materials). On success, emits `login_finish` via `InfoContext` (context handler injects `correlation_id`) with `account_thumb_hex`, `credential_id_hash`, and `sign_count`.
