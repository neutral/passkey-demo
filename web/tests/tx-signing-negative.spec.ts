...
test('options 409 → conflict error surfaced', async ({ page }) => {
  ...
  await expect(page.getByText('Error: conflict (nonce or credentials)')).toBeVisible()
})
...
test('options 400 → invalid bundle error surfaced', async ({ page }) => {
  ...
  await expect(page.getByText('Error: invalid bundle')).toBeVisible()
})
...
test('finish 409 → finish HTTP error surfaced', async ({ page }) => {
  ...
  await expect(page.getByText('Error: finish: HTTP 409')).toBeVisible()
})
