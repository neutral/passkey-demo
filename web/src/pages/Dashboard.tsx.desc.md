...
- Sign: uses `lib/api.postJson` to call `POST /tx/signing/options` with `{ bundle_cbor_b64 }`, transform to `PublicKeyCredentialRequestOptions`, call `navigator.credentials.get`, then `POST /tx/signing/finish`; on success, clear message and bundle, refresh list (which advances the nonce). If options returns 401 but `/me/account_key` still returns 200, surface a sender-key mismatch hint to rebuild the bundle. Non-2xx errors use the envelope `code` when present for user messaging.
