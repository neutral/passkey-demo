...
- `createRegistrationRoutes(config, deps)`: builds an Express router that logs `reg_options`, generates options via `@simplewebauthn/server`, enforces resident-key/UV policies, stores `{ challenge, rpID, origin, expiresAt }`, and surfaces `reg_session_id` + `expires_at` in JSON responses while mapping failures to standardized envelopes (`ERR_INTERNAL` on generator errors).
- Finish handler: calls `verifyRegistrationResponse`, enforces fmt `none` + UV, canonicalizes COSE keys via `cbor-x`, hashes accounts (`SHA-256("ACCTK1"||acct_cbor)`), writes `accounts`/`credentials` in a transaction, emits `reg_finish` logs with hashed identifiers, and logs failures via `reg_finish_error` with reasons before returning `ERR_BAD_REQUEST`/`ERR_UNAUTHORIZED`/`ERR_FORBIDDEN`/`ERR_CONFLICT` envelopes.

# Interactions
Mounted by `src/server.js` under `/authn/passkey/registration` after the 1 MiB body limit + rate limiter middleware. Depends on shared `logger.js` for structured logs, `error.js` for envelopes, and SQLite via injected `db` for persistence. Exports a shared session store reused across options/finish handlers. Login flow in step 7 reuses account hashing/logging helpers from this module.
