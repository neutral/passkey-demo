...
- Finish handler: calls `verifyRegistrationResponse`, enforces fmt `none` + UV, canonicalizes COSE keys via `cbor-x`, hashes accounts (`SHA-256("ACCTK1"||acct_cbor)`), writes `accounts`/`credentials` in a transaction, logs `reg_finish`, and returns `ERR_BAD_REQUEST`/`ERR_UNAUTHORIZED`/`ERR_FORBIDDEN`/`ERR_CONFLICT` responses via `error.js` helpers for policy and persistence failures.
