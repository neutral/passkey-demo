...
- `start()`: loads config, opens DB (PRAGMAs), applies migrations from `migrations.sql`, logs `db_init` (PRAGMAs), builds app, starts the server, and logs `server_start` and `listening` events.
