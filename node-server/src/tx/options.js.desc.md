...
- Calls `validateAndAnchorBundle` to canonicalize the bundle, enforce sender-key/nonce policies, and compute deterministic `challenge`/`tx_id` anchors; maps typed `BundleValidationError` kinds to `ERR_BAD_REQUEST`/`ERR_UNAUTHORIZED`/`ERR_CONFLICT` envelopes via `error.js` and logs success/failure (`tx_options`, `tx_options_error`) with correlation ids and hashed account identifiers.
