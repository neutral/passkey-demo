...
- Calls `validateAndAnchorBundle` to canonicalize the bundle, enforce sender-key/nonce policies, and compute deterministic `challenge`/`tx_id` anchors; maps typed `BundleValidationError` kinds to HTTP envelopes (400/401/409).
