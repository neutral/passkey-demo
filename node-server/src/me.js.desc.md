...
- Emits structured logs `me_account_key`/`me_account_key_error` with `account_thumb_hex` and `correlation_id` and maps unauthenticated/missing rows to the shared JSON envelope helper.
- Guards against malformed CBOR or missing coordinates by returning HTTP 500 envelopes rather than leaking partial state.
